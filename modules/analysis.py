...
def plot_scatterplot(df, x_column, y_column, title=\"Scatter Plot\", xlabel=\"X-axis\", ylabel=\"Y-axis\"):
    ...
    if df is None or x_column not in df.columns or y_column not in df.columns or \
       not pd.api.types.is_numeric_dtype(df[x_column]) or not pd.api.types.is_numeric_dtype(df[y_column]):
...
if __name__ == \"__main__\":
    ...
    if hist_img: print(\"Histogram for column A generated (Base64 string).\")
    else: print(hist_msg)

    # 測試散佈圖
    scatter_img, scatter_msg = plot_scatterplot(test_df, \"A\", \"B\")
    if scatter_img: print(\"Scatter plot for A vs B generated (Base64 string).\")
    else: print(scatter_msg)
